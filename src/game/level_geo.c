...
Gfx *geo_skybox_main(s32 callContext, struct GraphNode *node, UNUSED Mat4 *mtx) {
    ...
    if (callContext == GEO_CONTEXT_AREA_LOAD) {
        ...
        gfx = create_skybox_facing_camera(0, backgroundNode->background, camFrustum->fov, gCamera->pos[0],
                            gCamera->pos[1], gCamera->pos[2], gCamera->focus[0],
                            gCamera->focus[1], gCamera->focus[2]);
    }
